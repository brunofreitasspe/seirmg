...
| # | Lote | Escopo | Fonte |
|---|------|--------|-------|
| E3b | **Controle de Processos — rolagem infinita na pesquisa (Sei Pro)** | Substitui a paginação nativa por POST recursivo ao `frmProcedimentoControlar` com campo oculto de página (`hdnRecebidosPaginaAtual` etc.), acrescentando linhas retornadas. ⚠️ Risco médio, mesmo tratamento do Lote F — depende de nomes de campo ocultos internos do SEI, não verificável sem instância ao vivo; portar documentando o risco. | Sei Pro §2.3, código-fonte lido em `sei-pro.js:831-899` |
