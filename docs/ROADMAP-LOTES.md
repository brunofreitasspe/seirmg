...
| # | Lote | Escopo | Fonte |
|---|------|--------|-------|
| B | **Bloco de Assinatura — indicador visual + seleção em massa** | Badge colorido ao lado do logo, síncrono por carregamento de página (`verificarBlocoAssinatura.js`); botões de seleção em massa na tela do bloco (`selecionarDocumentosAssinar.js`). Complementa o monitoramento em background já entregue. | Sei++ §1.2 |
